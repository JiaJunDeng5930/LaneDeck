...
- `createHttpCenterQueryClient(options)` posts protocol `QueryRequest` payloads to the configured `queryUrl` and validates the current `QueryResponse` shape.

## Runtime Model

The app has three local collaborators: `query` for center reads, `shell` for iframe messages, and the document `#root` for rendered markup. Shell init supplies the full `centerQueryUrl` for hosted iframe execution. Dashboard rows prefer `eventText`, then `text`, then `message`; pick ids prefer `pickId`, then `sourceId`, then a deterministic row id.
